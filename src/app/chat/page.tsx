          ...
          {isEmpty ? (
            ...
            <div className="flex flex-col items-center text-center space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000">
              <div className="relative group">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-blue-500 to-indigo-600 flex items-center justify-center shadow-xl transform transition-all group-hover:scale-110 group-hover:rotate-3">
                  <span className="text-white text-2xl font-black tracking-tighter italic">Ai</span>
                </div>
                <div className="absolute -inset-2 rounded-3xl bg-blue-500/10 blur-md -z-10 animate-pulse" />
              </div>

              <div className="space-y-3">
                <h2 className="text-2xl font-bold tracking-tight text-slate-800">上午好，有什么我可以帮助你的吗？</h2>
                <p className="text-slate-400 text-[13px] font-medium">请从左侧选择知识库或直接向我提问</p>
              </div>
  ...
  function renderInputArea() {
    return (
      <div className="w-full space-y-5">
        {/* Main Input Box */}
        <div className="relative bg-white rounded-3xl border border-slate-200/80 shadow-[0_8px_30px_-6px_rgba(0,0,0,0.05)] focus-within:border-primary/50 focus-within:shadow-[0_12px_40px_-8px_rgba(59,130,246,0.15)] transition-all duration-300 overflow-hidden">
          
          {/* Mode Indicator Overlay */}
          {selectedMode && (
            <div className="flex items-center px-5 pt-4">
              <div className="flex items-center gap-2 px-2.5 py-1 bg-primary/5 border border-primary/15 rounded-full animate-in zoom-in-95 duration-200">
          ...
          <Textarea 
            ...
            className={cn(
              "min-h-[100px] max-h-[300px] border-none focus-visible:ring-0 text-[14px] font-medium resize-none px-6 bg-transparent placeholder:text-slate-300 transition-all",
              selectedMode ? "pt-2 pb-4" : "py-5"
            )}
            onKeyDown={(e) => {
          ...
          <div className="flex items-center justify-between px-5 pb-5 pt-1">
            <div className="flex items-center gap-1.5">
              <Button 
                variant="ghost" 
                size="sm" 
                className="h-8 px-3 rounded-xl bg-blue-50/80 text-primary hover:bg-blue-100/80 text-[11px] font-bold border border-blue-100/50 shadow-sm"
              >
                已选 {count} 项
                <ChevronDown className="h-3.5 w-3.5 ml-1 opacity-60" />
              </Button>
              <div className="h-4 w-[1px] bg-slate-100 mx-1.5" />
              <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-all rounded-lg active:scale-90">
            ...
            <Button 
              ...
              size="sm" 
              className="bg-primary hover:bg-primary/95 text-white rounded-2xl h-9 px-6 text-[13px] font-bold transition-all shadow-lg shadow-primary/25 active:scale-95 disabled:shadow-none"
            >
              发送
              <Send className="h-3.5 w-3.5 ml-2.5" />
            </Button>
          </div>
        </div>

        {/* Tool Pills Bar */}
        <div className="flex items-center justify-center gap-3 overflow-x-auto scrollbar-hide py-1 px-4">
          {AI_TOOLS.map((tool) => (
            <button 
              key={tool.label}
              onClick={() => setSelectedMode(tool.mode)}
              className={cn(
                "flex items-center gap-2.5 px-5 py-2.5 bg-white border rounded-full text-[12px] font-bold transition-all shadow-sm whitespace-nowrap active:scale-95 group",
